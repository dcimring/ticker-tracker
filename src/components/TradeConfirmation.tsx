...
export default function TradeConfirmation({ trades, onConfirm, onCancel }: TradeConfirmationProps) {
  return (
    <div className="fixed inset-0 bg-surface/80 backdrop-blur-xl z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-surface-container-high w-full max-w-xl rounded-[3rem] shadow-2xl border border-outline-variant overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-10 bg-surface-container-high">
        ...
        <div className="max-h-[50vh] overflow-y-auto p-10 space-y-6 bg-surface-container">
        ...
        <div className="p-10 flex items-center gap-6 bg-surface-container-high">
          <button
            onClick={onCancel}
            className="flex-1 px-8 py-5 rounded-[1.5rem] bg-surface-container-highest text-on-surface-variant font-black uppercase tracking-widest text-[10px] hover:bg-surface-bright/20 transition-all"
          >
            Discard
          </button>
          <button
            onClick={() => onConfirm(trades)}
            className="flex-1 px-8 py-5 rounded-[1.5rem] bg-gradient-to-br from-primary to-primary-container text-on-primary-container font-black uppercase tracking-widest text-[10px] hover:opacity-90 transition-all shadow-xl shadow-primary/10"
          >
            Commit to Watchlist
          </button>
        </div>
